...
SDL_AppResult SDL_AppInit(void** appstate, int /*argc*/, char* /*argv*/[]) {
  SDL_SetAppMetadata("SpinningTriangle", "1.0", "com.umutech.sdl3vk");
  ...
  auto spinning_triangle = new (std::nothrow) SpinningTriangle;
  if (!spinning_triangle) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                 "Failed to create SpinningTriangle");
    return SDL_APP_FAILURE;
  }
  if (!spinning_triangle->Initialize()) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                 "Failed to initialize SpinningTriangle");
    return SDL_APP_FAILURE;
  }
  *appstate = spinning_triangle;
...
SDL_AppResult SDL_AppEvent(void* appstate, SDL_Event* event) {
  switch (event->type) {
    ...
    case SDL_EVENT_WINDOW_MINIMIZED:
      if (auto vk_triangle = static_cast<SpinningTriangle*>(appstate)) {
