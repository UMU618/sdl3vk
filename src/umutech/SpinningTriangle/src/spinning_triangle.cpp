...
// UMU: w = 1.0 means that it's a point, not a direction.
constexpr std::array<Vertex, 3> kVertices{
    Vertex{{0.0f, -0.5f, 0.0f, 1.0f}, {1.0f, 0.0f, 0.0f}},
...
bool SpinningTriangle::Initialize() noexcept {
  window_ = SDL_CreateWindow("SpinningTriangle", 800, 618,
...
void SpinningTriangle::Free() noexcept {
  if (!window_) {
    return;
  }
...
bool SpinningTriangle::Draw() noexcept {
  assert(window_);
  assert(device_);
  ...
  if (!RecordCommandBuffer(image_index)) {
    return false;
  }
...
bool SpinningTriangle::RecreateSwapchain() noexcept {
...
// 1
bool SpinningTriangle::CreateInstance() noexcept {
...
// 2
bool SpinningTriangle::CreateSurface() noexcept {
...
// 3
bool SpinningTriangle::PickPhysicalDevice() noexcept {
...
// 4
bool SpinningTriangle::CreateLogicalDevice() noexcept {
...
// 5
bool SpinningTriangle::CreateSwapchain() noexcept {
...
// 6
bool SpinningTriangle::CreateImageViews() noexcept {
...
// 7
bool SpinningTriangle::CreateRenderPass() noexcept {
...
// 8
bool SpinningTriangle::CreateShaderModules() noexcept {
  const auto assets_dir = GetProductDirectory() / "assets";

  auto vert_shader_code = ReadFile(assets_dir / "spinning_triangle_vert.spv");
  ...
  auto frag_shader_code = ReadFile(assets_dir / "spinning_triangle_frag.spv");
...
// 9
bool SpinningTriangle::CreateGraphicsPipeline() noexcept {
  ...
  // UMU: Use vk::CullModeFlagBits::eNone to see the triangle from both sides
...
// 10
bool SpinningTriangle::CreateFramebuffers() noexcept {
...
// 11
bool SpinningTriangle::CreateCommandPool() noexcept {
...
// 12
bool SpinningTriangle::CreateVertexBuffer() noexcept {
...
// 13
bool SpinningTriangle::CreateCommandBuffers() noexcept {
...
// Draw
bool SpinningTriangle::RecordCommandBuffer(std::uint32_t image_index) noexcept {
  ...
  // Calculate the model-view-projection matrix for rotating the triangle
  glm::mat4 model = glm::rotate(glm::mat4(1.0f), glm::radians(rotation_angle_),
                                glm::vec3(0.0f, -1.0f, 0.0f));
  rotation_angle_ += 1.0f;
  if (360.0f <= rotation_angle_) {
  ...
  glm::mat4 view =
      glm::lookAt(glm::vec3(0.0f, 0.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f),
                  glm::vec3(0.0f, 1.0f, 0.0f));
  float aspect_ratio =
      static_cast<float>(swapchain_extent_.width) / swapchain_extent_.height;
  glm::mat4 projection =
...
// 14
bool SpinningTriangle::CreateSynchronizationObjects() noexcept {
